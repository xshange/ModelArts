...
// ../github-shared/utils.mjs
import { createHash } from "crypto";
import { existsSync, readFileSync, writeFileSync, mkdirSync } from "fs";
import { join } from "path";
var CACHE_DIR = "_build/cache";
var MS_PER_DAY = 24 * 60 * 60 * 1e3;
var DEFAULT_CACHE_TTL = MS_PER_DAY;
function createCache(prefix, ttl = DEFAULT_CACHE_TTL) {
...
var cache = createCache("simple-icon", 30 * MS_PER_DAY);
  ...
  directives: [
    {
      ...
      body: { type: String },
      run(data) {
        const parsed = data.body ? jsYaml.load(data.body) : {};
  ...
  transforms: [
    {
      ...
      plugin: () => async (tree) => {
        ...
        for (const node of footers) {
          ...
          const { type: type2, position, ...fields } = node;
          Object.keys(node).forEach((k) => delete node[k]);
          Object.assign(node, {
            type: "anywidget",
            // Replaced with release URLs at build time (see build.mjs)
            esm: "https://github.com/jupyter-book/myst-plugins/releases/download/footer-latest/widget.mjs",
            css: "https://github.com/jupyter-book/myst-plugins/releases/download/footer-latest/footer.css",
